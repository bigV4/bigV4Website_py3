...
def preprocessing():
    ...
    for file in fileDist_train:
        #取q1ap_20121218281.png前的部分，作为标签，将路径和标签中间隔一个空格后逐行写入.txt文件中
...
if not os.path.exists('dists'):
    os.makedirs('dists')
if not os.path.isfile('dists/train.txt'):
    open('dists/train.txt', "w")
if not os.path.isfile('dists/test.txt'):
    open('dists/test.txt', "w")
